...
playback) *and* an HTTP dispatch endpoint the edge calls: an inbound
:class:`~petbot.domain.call.SkillCall` is run by a :class:`~petbot.platform.Worker`
hosting a single :class:`~petbot.skills.music.MusicSkill`, whose
...
def run(settings: MusicSettings) -> None:
    ...
    log.info("Starting PetBot music worker.")
